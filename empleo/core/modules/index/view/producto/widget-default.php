...
<?php foreach($cats as $cat):?>
  ...
  <a href="index.php?view=Profesiones&cat=<?php echo $cat->short_name; ?>" class="list-group-item"><i class="fa fa-chevron-right"></i>  <?php echo $cat->name; ?></a>
<?php endforeach; ?>
</div>
<?php endif; ?>
      </div>
...
<a href="index.php?action=addtocart&product_id=<?php echo $p->id; ?>&href=product" class="btn btn-labeled btn-primary tip" title="A&ntilde;adir al carrito">
                <span><i class="fas fa-hard-hat"></i></i></span> Solicitar servicio</a>
<?php else:?>
...
<?php endif; ?>
    <?php if($p->in_existence):?>
      <p class="text-success">Servicio en Existencia</p>
    <?php else:?>
      <p class="text-warning">Servicio no disponible</p>
    <?php endif; ?>
  </div>
  </div>
  <br><br>
